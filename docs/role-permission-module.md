...
1. **`SettingsController`-এ Policy নেই।** `authorizeResource(Option::class, 'option')` করা হয়েছে কিন্তু `app/Policies/` -এ `OptionPolicy` নেই। ফলে `index` → `viewAny` ability চেক হয়, `Gate::before` `hasPermission('viewAny')` খোঁজে (যা কোনো permission নয়), তাই `settings.view` / `settings.edit` permission গুলো আসলে কোথাও এনফোর্স হচ্ছে না — এবং policy না থাকায় চেকটা অপ্রত্যাশিত আচরণ করতে পারে। ঠিক করার সহজ উপায়: `OptionPolicy` বানিয়ে `viewAny → settings.view`, `update → settings.edit` ম্যাপ করা, অথবা route-এ `permission:settings.view` middleware দেওয়া।
